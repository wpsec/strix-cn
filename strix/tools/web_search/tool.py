...
def _do_search(query: str) -> dict[str, Any]:
    ...
    outcome = _guarded_call(
        "Web search",
        (
            "Web 搜索拒绝了该查询，请缩短查询、补充具体上下文并移除异常字符后重试"
        ),
        fetch,
    )
...
def _do_get_contents(urls: list[str]) -> dict[str, Any]:
    ...
    if not cleaned:
        return {"success": False, "error": "请至少提供一个 URL"}
    if len(cleaned) > _EXA_MAX_CONTENT_URLS:
        return {
            "success": False,
            "error": f"URL 数量过多，每次最多提供 {_EXA_MAX_CONTENT_URLS} 个",
        }
    ...
    if integrations.web_search_provider == "perplexity":
        ...
        return {
            "success": False,
            "error": (
                "页面全文抓取需要 Exa Provider "
                "（当前已将 STRIX_WEB_SEARCH_PROVIDER 固定为 perplexity），请改用 web_search"
            ),
        }
    ...
    if missing:
        ...
        result["warning"] = (
            f"Exa 未返回 {len(cleaned)} 个 URL 中 {len(missing)} 个页面的内容，"
            "这些页面未包含在下面的结果中"
        )
    return result
