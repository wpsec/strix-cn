...
func TestVulnerabilityReportRendersCalibrationFields(t *testing.T) {
	out := ansi.Strip(Tool(tool("create_vulnerability_report",
		...
		"completed")))
	requireContains(t, out,
		"置信度", "MEDIUM", "no authenticated instance to replay against",
		"反证", "gateway may strip the id parameter",
		"严重度变化条件", "other tenants' bank details",
		"修复验证", "bypass review reasoned only",
	)
}
