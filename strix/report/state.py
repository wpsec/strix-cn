...
class ReportState:
    ...
    def add_vulnerability_report(
        ...
        if isinstance(http_request, str) and http_request.strip():
    ...
    def update_vulnerability_report(
        ...
        *,
        ...
        for key, raw_value in fields.items():
            ...
            if isinstance(value, str):
                ...
                if key in _LOWERCASE_REPORT_FIELDS:
                    value = value.lower()
                if not value or (
                    key in {"burp_request", "http_request", "http_response"}
                    and not value.strip()
                ):
                    continue
        ...
        # Persist the revision before mutating in-memory evidence so a failed
        # callback leaves the previous report retryable.
