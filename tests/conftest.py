...
@pytest.fixture(autouse=True)
def _isolate_cli_config(tmp_path_factory: pytest.TempPathFactory) -> None:
    """Keep the whole suite from reading or writing the developer's real config.

    ``load_settings`` merges ``~/.strix/cli-config.json``'s env block, so on a
    machine already pointed at a real gateway, tests resolve routes the test
    never configured (an Anthropic-protocol base re-routes bare model names),
    and setup-persistence paths rewrite the file mid-run. Point the loader at
    a nonexistent per-test path; tests exercising the config loader call
    ``apply_config_override`` themselves afterwards and so win.
    """
    from strix.config import loader

    missing = tmp_path_factory.mktemp("config-isolation") / "cli-config.json"
    loader.apply_config_override(missing)
