...
def _run_config(base_url: str, **kwargs: Any) -> RunConfig:
    ...
    tool_not_found_behavior = kwargs.pop("tool_not_found_behavior", None)
    run_config = RunConfig(model_provider=_Provider(), **kwargs)
    if tool_not_found_behavior is not None:
        setattr(run_config, "tool_not_found_behavior", tool_not_found_behavior)
    return run_config
